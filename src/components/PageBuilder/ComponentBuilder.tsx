...
const ComponentBuilder: React.FC = () => {
  ...
  const addPropertyFromSelection = () => {
    ...
    if (isArrayField && arrayParentName && arrayFieldName) {
      ...
      // 配列タイプの場合、li要素内のすべての子要素を自動検出してフィールドを作成
      if (newPropType === 'array') {
        const arrayFieldId = `prop_${Date.now()}`;
        const arrayDefaultValue = getDefaultValue('array', localSelectedText);
        
        // 配列フィールドを作成
        const arrayField: PropField = {
          id: arrayFieldId,
          name: autoGeneratedName,
          type: 'array',
          label: generateLabel(autoGeneratedName),
        ...
        setPropFields([...propFields, arrayField]);

        // li要素内の子要素を検出してフィールドを作成
        const liMatches = localSelectedText.match(/<li[^>]*>([\s\S]*?)<\/li>/gi);
        if (liMatches && liMatches.length > 0) {
          // 最初のli要素を解析して子要素の構造を把握
          const firstLiContent = liMatches[0].replace(/<\/?li[^>]*>/gi, '');
          
          // 子要素を検出する関数
          const detectChildElements = (html: string): Array<{ type: 'link' | 'text' | 'other', tagName?: string, content: string, fullMatch: string }> => {
            const elements: Array<{ type: 'link' | 'text' | 'other', tagName?: string, content: string, fullMatch: string }> = [];
            ...
            // その他のタグ（span, strong, em, etc.）を検出
            const otherTagMatches = html.match(/<(span|strong|em|b|i|p|div)[^>]*>([\s\S]*?)<\/\1>/gi);
            if (otherTagMatches) {
              otherTagMatches.forEach(match => {
                const tagMatch = match.match(/<(\w+)/);
                if (tagMatch && !match.includes('<a')) { // <a>タグは既に処理済み
                  elements.push({
                    type: 'other',
                    tagName: tagMatch[1],
                    content: match,
            ...
            // テキストノードを検出（タグで囲まれていないテキスト）
            const textOnlyMatch = html.replace(/<[^>]+>/g, '').trim();
            if (textOnlyMatch && !html.match(/<[^>]+>/)) {
              elements.push({
                type: 'text',
                content: textOnlyMatch,
                fullMatch: textOnlyMatch
              });
            }
          ...
          const childElements = detectChildElements(firstLiContent);
          
          // 検出された子要素ごとにフィールドを作成
          const newFields: PropField[] = [];
          let updatedHtml = localSelectedText;
          
          childElements.forEach((child, index) => {
            const fieldName = `${autoGeneratedName}_${child.type}_${index + 1}`;
            let fieldType: PropField['type'] = 'text';
            let defaultValue: any = '';
            
            if (child.type === 'link') {
              fieldType = 'link';
              const hrefMatch = child.content.match(/href=["']([^"']*)["']/i);
              const targetMatch = child.content.match(/target=["']([^"']*)["']/i);
              const url = hrefMatch ? hrefMatch[1] : '';
              const target = targetMatch ? targetMatch[1] : '_self';
              const linkTextMatch = child.content.match(/>([\s\S]*?)<\/a>/i);
              const linkText = linkTextMatch ? getTextContent(linkTextMatch[1]).trim() : '';
              ...
              fieldType = 'textarea';
              defaultValue = getTextContent(child.content);
            }
            
            const childField: PropField = {
              id: `prop_${Date.now()}_${index}`,
              name: fieldName,
              type: fieldType,
              label: `${generateLabel(autoGeneratedName)} - ${child.type === 'link' ? 'リンク' : child.type === 'text' ? 'テキスト' : child.tagName || '要素'} ${index + 1}`,
              defaultValue: defaultValue,
              description: '',
              position: localRange,
              elementPath: elementPath,
              arrayFieldName: fieldName,
              arrayParentId: arrayFieldId,
            };
            
            newFields.push(childField);
            
            // HTMLにdata-prop属性を追加
            const escapedFullMatch = child.fullMatch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const dataPropAttr = ` data-prop="${autoGeneratedName}" data-bind-type="array" data-array-field="${fieldName}"`;
            
            // 子要素の開始タグに属性を追加
            if (child.type === 'link' || child.type === 'other') {
              const tagMatch = child.fullMatch.match(/^<(\w+)([^>]*)>/);
              if (tagMatch) {
                const tagName = tagMatch[1];
                let existingAttrs = tagMatch[2] || '';
                ...
                const newTag = `<${tagName}${existingAttrs}${dataPropAttr}>`;
                const restOfTag = child.fullMatch.slice(tagMatch[0].length);
                const newFullMatch = newTag + restOfTag;
                
                updatedHtml = updatedHtml.replace(new RegExp(escapedFullMatch, 'g'), newFullMatch);
              }
            } else if (child.type === 'text') {
              // テキストノードの場合は、親のli要素に属性を追加するか、spanで囲む
              // ここでは、li要素内のテキストをspanで囲む処理を追加
              const textWithSpan = `<span${dataPropAttr}>${child.content}</span>`;
              updatedHtml = updatedHtml.replace(new RegExp(escapedFullMatch, 'g'), textWithSpan);
            }
          });
          
          // 配列のdefaultValueを更新（検出されたフィールドを含む）
          const updatedArrayValue = Array.isArray(arrayDefaultValue) ? arrayDefaultValue.map((item: any, itemIndex: number) => {
            const itemObj: any = {};
            childElements.forEach((child, index) => {
              const fieldName = `${autoGeneratedName}_${child.type}_${index + 1}`;
              if (typeof item === 'object' && item !== null) {
                itemObj[fieldName] = item[fieldName] || (child.type === 'link' ? { url: '', text: '', target: '_self' } : '');
              } else {
                itemObj[fieldName] = child.type === 'link' ? { url: '', text: '', target: '_self' } : '';
              }
            });
            return itemObj;
          }) : [];
          ...
          setPropFields(prevFields => {
            ...
            const arrayFieldIndex = updatedFields.findIndex(f => f.id === arrayFieldId);
            if (arrayFieldIndex >= 0) {
              updatedFields[arrayFieldIndex] = { ...updatedFields[arrayFieldIndex], defaultValue: updatedArrayValue };
            }
            return [...updatedFields, ...newFields];
          });
          
          // HTMLコードを更新
          const dataPropAttr = ` data-prop="${autoGeneratedName}" data-bind-type="array"`;
          const openingTagMatch = localSelectedText.match(/^<([a-zA-Z][a-zA-Z0-9]*)((?:\s[^>]*)?)>/);
          if (openingTagMatch) {
            const tagName = openingTagMatch[1];
            let existingAttrs = openingTagMatch[2] || '';
            const restOfElement = updatedHtml.slice(openingTagMatch[0].length);
